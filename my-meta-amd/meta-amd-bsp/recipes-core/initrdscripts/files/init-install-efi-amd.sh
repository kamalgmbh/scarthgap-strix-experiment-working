...
if [ -f /run/media/$1/isolinux/isolinux.cfg ]; then
    root_part_uuid=$(blkid -o value -s PARTUUID ${rootfs})
    GRUBCFG="$EFIDIR/grub.cfg"
    cp /run/media/$1/isolinux/isolinux.cfg $GRUBCFG
...
elif [ -f /run/media/$1/EFI/BOOT/grub.cfg ]; then
    root_part_uuid=$(blkid -o value -s PARTUUID ${rootfs})
    GRUBCFG="$EFIDIR/grub.cfg"
...
for types in bzImage zImage vmlinux vmlinuz fitImage xen mboot libcom32 ldlinux libutil vesamenu initrd isolinux.bin isolinux.cfg; do
